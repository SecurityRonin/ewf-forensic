...
fn analyse_and_compute_hashes_anomalies_match_analyse() {
    ...
    let (combined_anomalies, _hashes) = EwfIntegrityPath::from_path(f.path())
        ...
        .unwrap();
    let standalone_anomalies = EwfIntegrityPath::from_path(f.path()).analyse().unwrap();
    assert_eq!(
...
fn analyse_and_compute_hashes_hashes_match_compute_hashes() {
    ...
    assert_eq!(
        combined_hashes.sha1, standalone_hashes.sha1,
        "SHA-1 mismatch"
    );
    assert_eq!(
        combined_hashes.sha256, standalone_hashes.sha256,
        "SHA-256 mismatch"
    );
}
...
fn analyse_and_compute_hashes_anomalies_and_hashes_on_tampered_image() {
    ...
    assert!(
        result.is_ok(),
        "expected Ok even on tampered image, got: {:?}",
        result.err()
    );
