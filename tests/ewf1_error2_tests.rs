...
fn e01_with_error2(count: u32) -> Vec<u8> {
    ...
    let error2_body: Vec<u8> = {
        ...
        v.extend(std::iter::repeat(0u8).take(count as usize * ENTRY_SIZE));
        v
    };
    ...
    buf.extend_from_slice(&make_section_descriptor("volume", error2_off, volume_section_size));
    buf.extend(std::iter::repeat(0u8).take(VOLUME_DATA_SIZE));

    buf.extend_from_slice(&make_section_descriptor("error2", done_off, error2_section_size));
...
fn error2_count_value_is_correct() {
    ...
    if let Some(EwfIntegrityAnomaly::BadSectorsPresent { count }) = anomaly {
        assert_eq!(*count, 5, "BadSectorsPresent count must equal error2 entry_count");
    } else {
