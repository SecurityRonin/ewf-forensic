...
fn invalid_signature_display() {
    ...
    assert!(s.to_lowercase().contains("signature"), "missing 'signature': {s:?}");
}
...
fn segment_number_zero_display() {
    ...
    assert!(s.contains("segment") || s.contains("zero") || s.contains("0"), "{s:?}");
}
...
fn section_descriptor_crc_mismatch_display() {
    ...
    assert!(s.contains("0x100") || s.contains("100"), "missing offset: {s:?}");
    assert!(s.contains("dead") || s.contains("DEAD") || s.contains("0xdead") || s.contains("0xDEAD"), "missing computed: {s:?}");
}
...
fn section_chain_broken_display() {
    let s = format!(
        "{}",
        EwfIntegrityAnomaly::SectionChainBroken { at_offset: 0x80, next_offset: 0 }
    );
    assert!(!is_rust_debug(&s));
    assert!(s.to_lowercase().contains("chain") || s.to_lowercase().contains("broken"), "{s:?}");
}
...
fn hash_mismatch_display() {
    ...
    assert!(s.to_lowercase().contains("ab") || s.to_lowercase().contains("abab"), "missing computed: {s:?}");
    assert!(s.to_lowercase().contains("cd") || s.to_lowercase().contains("cdcd"), "missing stored: {s:?}");
}
...
fn hash_section_missing_display() {
    ...
    assert!(!is_rust_debug(&s));
    assert!(s.to_lowercase().contains("hash") || s.to_lowercase().contains("md5"), "{s:?}");
}
...
fn chunk_decompression_error_display() {
    ...
    assert!(
        s.to_lowercase().contains("decompress") || s.to_lowercase().contains("corrupt") || s.to_lowercase().contains("zlib"),
        "{s:?}"
    );
}
...
fn digest_sha1_mismatch_display() {
    ...
    assert!(!is_rust_debug(&s));
    assert!(s.to_lowercase().contains("sha") || s.to_lowercase().contains("sha-1") || s.to_lowercase().contains("sha1"), "{s:?}");
...
fn external_md5_mismatch_display() {
    ...
    assert!(!is_rust_debug(&s));
    assert!(s.to_lowercase().contains("md5") || s.to_lowercase().contains("custody") || s.to_lowercase().contains("reference"), "{s:?}");
...
fn external_sha256_mismatch_display() {
    ...
    assert!(!is_rust_debug(&s));
    assert!(s.to_lowercase().contains("sha-256") || s.to_lowercase().contains("sha256"), "{s:?}");
...
fn ewf2_encrypted_section_display() {
    let s = format!("{}", EwfIntegrityAnomaly::Ewf2EncryptedSection { offset: 0x200 });
    assert!(!is_rust_debug(&s));
    assert!(s.to_lowercase().contains("encrypt"), "{s:?}");
    assert!(s.contains("0x200") || s.contains("200") || s.contains("512"), "{s:?}");
}
...
fn ewf2_hash_section_missing_display() {
    ...
    assert!(!is_rust_debug(&s));
    assert!(s.to_lowercase().contains("hash") || s.to_lowercase().contains("ewf v2") || s.to_lowercase().contains("v2"), "{s:?}");
}
...
fn segment_out_of_order_display() {
    let s = format!(
        "{}",
        EwfIntegrityAnomaly::SegmentOutOfOrder { segment_number: 3, expected: 2 }
    );
    assert!(!is_rust_debug(&s));
    assert!(s.contains("3") && s.contains("2"), "missing segment numbers: {s:?}");
}
...
fn display_does_not_contain_struct_braces() {
    ...
    for a in &anomalies {
        ...
        assert!(!s.contains(" { "), "Display looks like Debug for {a:?}: {s:?}");
        // Should not be empty
