...
impl E01Builder {
    ...
    pub fn with_nonfinal(mut self) -> Self {
        self.nonfinal = true;
        self
    }
    /// Override `chunk_count` in the volume section (for multi-segment: set to total image chunks).
    pub fn with_volume_chunk_count(mut self, n: u32) -> Self {
        self.volume_chunk_count_override = Some(n);
        self
    }
    /// Insert a "digest" section with the given SHA-1 (MD5 computed from actual sectors data).
    pub fn with_digest_sha1(mut self, sha1: [u8; 20]) -> Self {
        self.digest_sha1_override = Some(sha1);
        self
    }
    ...
    pub fn with_corrupt_chunk_checksum(mut self, idx: usize) -> Self {
        self.corrupt_chunk_checksum = Some(idx);
        self
    }
    ...
    pub fn build(self) -> Vec<u8> {
        ...
        let chunk_stride = if self.chunk_checksums {
            chunk_size + 4
        } else {
            chunk_size
        };
        ...
        if self.nonfinal {
            ...
            if self.insert_gap {
                buf.extend_from_slice(&[
                    0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE, 0x13, 0x37, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x01,
                ]);
            }
            if self.insert_zero_gap {
            ...
            if let Some(sha1_bytes) = self.digest_sha1_override {
                let next_after_digest = if self.omit_hash {
                    done_desc_off
                } else {
                    hash_desc_off
                };
...
fn make_e01_with_full_volume(media_type: u8, guid: Option<[u8; 16]>, corrupt_crc: bool) -> Vec<u8> {
    const SPC: u32 = 64;
    const BPS: u32 = 512;
    ...
    let vol_body = make_full_volume_body(
        CHUNK_COUNT,
        SPC,
        BPS,
        sector_count,
    ...
    let table_section_size =
        SECTION_DESCRIPTOR_SIZE as u64 + table_header_size + table_entries_size;
