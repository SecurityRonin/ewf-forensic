...
fn multiseg_v1_md5_matches() {
    ...
    assert!(
        !findings
            .iter()
            .any(|a| matches!(a, ewf_forensic::EwfIntegrityAnomaly::ExternalMd5Mismatch { .. })),
...
fn multiseg_v1_sha1_matches() {
    ...
    assert!(
        !findings
            .iter()
            .any(|a| matches!(a, ewf_forensic::EwfIntegrityAnomaly::ExternalSha1Mismatch { .. })),
...
fn multiseg_v1_computed_hashes_match() {
    ...
    assert_eq!(md5_hex, MD5, "computed MD5 must match ewfverify ground truth");
    assert_eq!(sha1_hex, SHA1, "computed SHA-1 must match ewfverify ground truth");
}
